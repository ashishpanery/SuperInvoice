...
function saveandPrintInvoice(){
			...
			for(var i=1;i<count;i++){
				...
				if( (code == undefined || code == "New Item") && (itemName == undefined || itemName == "") ){
					//code to hide the empty row in invoice.
					//$("#lineItem\\."+i).hide();
					continue;
				}
				...
				var ta = $('#taxvalue\\.'+i).val();
				if(ta == "")
					ta = "0.0";
				taxAmt = taxAmt + parseFloat(ta);
...
function saveandPrintInvoice_hold(){
			var invoiceNo = $("#invNumber").val();
			var invoiceDate = $('#invoiceDate').val();
			var dueDate = $('#dueDate').val();
...
CreateInvoiceUtil.calculateInvoiceTotal = function() {
	...
	for ( var r = 1; r <= CreateInvoiceUtil.TOTAL_LINE_ITEMS; r++) {
		...
		console.log("r="+$("#discvalue\\." + r).val());
		totalDiscount = totalDiscount + parseFloat($("#discvalue\\." + r).val());
	}
	e = e.toFixed("2");
	$("#subTotal").html(e);
	...
	for(var tax in taxes){
		...
		'<td><input type="text" id="'+tax+'_val" value="'+taxes[tax]+'" class="number_value"></td></tr>';
	}
	taxTable += '<tr><td><input type="text" id="round_off" value="Round Off" style=""></td>'+
	'<td><input type="text" id="round_off_val" value="" class="number_value"></td></tr>';
...
$("#total").html(Math.round(o));
	//console.log((Math.ceil(o)-o).toFixed(2));
	$('#round_off_val').val((Math.round(o)-o).toFixed(2));
};
